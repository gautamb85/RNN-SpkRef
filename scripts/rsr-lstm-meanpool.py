...
project_path='/misc/scratch03/reco/bhattaga/data/i-vectors/dnn_projects/rsr/rnn-utt-lstm-meanpool/'
...
network['lstm-forward'] = lasagne.layers.recurrent.LSTMLayer(network['input'],300, mask_input=network['mask'],
                          ingate=gate_parameters, forgetgate=gate_parameters, cell=cell_parameters, outgate=gate_parameters,
                          grad_clipping=100., only_return_final=False)

network['lstm-backward'] = lasagne.layers.recurrent.LSTMLayer(network['input'],300, mask_input=network['mask'],
                           ingate=gate_parameters, forgetgate=gate_parameters, cell=cell_parameters, outgate=gate_parameters, 
                           grad_clipping=100., only_return_final=False, backwards=True)


network['concat'] = ConcatLayer([network['lstm-forward'],network['lstm-backward']],axis=2)
#print lasagne.layers.get_output(network['concat']).eval({X: x_test, Masks:x_mask}).shape

#added for lstm-dropout
network['mean-pool'] = lasagne.layers.ExpressionLayer(network['concat'], lambda X: X.mean(axis=1,keepdims=True), lambda s: (s[0],1)+s[1:])
#print lasagne.layers.get_output(network['mean-pool']).eval({X: x_test,Masks:x_mask}).shape
...
network['fc2'] = DenseLayer(network['mean-pool'],5820,nonlinearity=None)
network['prob'] = NonlinearityLayer(network['fc2'],nonlinearity=softmax)

network_output = lasagne.layers.get_output(network['prob'])
  
hidden_output = lasagne.layers.get_output(network['mean-pool'], deterministic=True)
...
lr_decay = np.array(0.25, dtype='float32')
...
while 'true':
    ...
    scheme = ShuffledScheme(examples=train_set.num_examples, batch_size=16)
    scheme1 = SequentialScheme(examples=valid_set.num_examples, batch_size=16)
