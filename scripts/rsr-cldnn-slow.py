...
network['conv1'] = batch_norm(ConvLayer(network['reshape'], 256,(9,9),pad='same',flip_filters=False,)) 
...
network['conv2'] = batch_norm(ConvLayer(network['pool1'],256,(3,4),pad=(1,0), flip_filters=False,))
#print lasagne.layers.get_output(network['conv2']).eval({X: x_test}).shape

network['reshape1'] = ReshapeLayer(network['conv2'],(batchs*tsteps,[1],[3]))
#print lasagne.layers.get_output(network['reshape1']).eval({X: x_test}).shape

network['lproj'] = batch_norm(DenseLayer(network['reshape1'],256,nonlinearity=None))
#print lasagne.layers.get_output(network['lproj']).eval({X: x_test}).shape

network['lstm-input'] = ReshapeLayer(network['lproj'],(batchs,tsteps,[1]))
...
network['lstm-forward'] = lasagne.layers.recurrent.LSTMLayer(network['lstm-input'],400, mask_input=network['mask'],
...
network['lstm-backward'] = lasagne.layers.recurrent.LSTMLayer(network['lstm-input'],400, mask_input=network['mask'],
                           ingate=gate_parameters, forgetgate=gate_parameters, cell=cell_parameters, outgate=gate_parameters, 
...
#print lasagne.layers.get_output(network['concat']).eval({X: x_test, Masks:x_mask}).shape
