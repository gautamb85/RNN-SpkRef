...
project_path='/misc/scratch03/reco/bhattaga/data/i-vectors/dnn_projects/rsr/rnn-utt-cldnn/'
...
train_set = H5PYDataset('/misc/scratch03/reco/bhattaga/data/RSR/rsr-DEVBKG-cldnn-train.hdf5',which_sets=('train',),subset=slice(0,16))
h=train_set.open()
data = train_set.get_data(h,slice(0,train_set.num_examples))

x_test = data[0]
#x_test = np.reshape(x_test,(3,x_test.shape[0],x_test.shape[1]))
...
network['input'] = InputLayer(shape=(None,382,400), input_var = X)
batchs,tsteps,fdim = network['input'].input_var.shape

#network['reshape'] = ReshapeLayer(network['input'],(batchs,382,40,10))
#print lasagne.layers.get_output(network['reshape']).eval({X: x_test}).shape

network['conv1'] = batch_norm(ConvLayer(network['input'], 256,(9,9),pad='valid',flip_filters=False,)) 
...
network['conv2'] = batch_norm(ConvLayer(network['pool1'],256,(4,3),pad='valid', flip_filters=False,))
print lasagne.layers.get_output(network['conv2']).eval({X: x_test}).shape

network['lproj'] = batch_norm(DenseLayer(network['conv2'],256,nonlinearity=None))
print lasagne.layers.get_output(network['lproj']).eval({X: x_test}).shape

network['lstm-input'] = ReshapeLayer(network['lproj'],(batchs,tsteps,256))
print lasagne.layers.get_output(network['lstm-input']).eval({X: x_test}).shape
...
network['lstm-forward'] = lasagne.layers.recurrent.LSTMLayer(network['lstm-input'],500, mask_input=network['mask'],
...
network['lstm-backward'] = lasagne.layers.recurrent.LSTMLayer(network['lstm-input'],500, mask_input=network['mask'],
                           ingate=gate_parameters, forgetgate=gate_parameters, cell=cell_parameters, outgate=gate_parameters, 
...
network['drop1'] = DropoutLayer(network['fc1'],p=0.3)
...
network['fc2'] = DenseLayer(network['fc1'],5820,nonlinearity=None)
network['prob'] = NonlinearityLayer(network['fc2'],nonlinearity=softmax)
...
train_set = H5PYDataset('/misc/scratch03/reco/bhattaga/data/RSR/rsr-DEVBKG-cldnn-train.hdf5',which_sets=('train',),subset=slice(0,128))
valid_set = H5PYDataset('/misc/scratch03/reco/bhattaga/data/RSR/rsr-DEVBKG-cldnn-train.hdf5',which_sets=('valid',),subset=slice(0,128))
...
while 'true':
    ...
    for data in train_stream.get_epoch_iterator():
        ...
        t_data,t_labs,t_mask,t_name = data
        t_data = np.swapaxes(t_data,1,2)
    ...
    for data in valid_stream.get_epoch_iterator():
        v_data,v_labs,v_mask,t_name = data    
        v_data = np.swapaxes(v_data,1,2)
