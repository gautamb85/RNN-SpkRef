...
project_path='/misc/scratch03/reco/bhattaga/data/i-vectors/dnn_projects/rsr/rnn-utt-gru/'
...
network['gru-forward'] = lasagne.layers.recurrent.GRULayer(network['input'],400, mask_input=network['mask'], gradient_steps=80, grad_clipping=100., only_return_final=True)

network['gru-backward'] = lasagne.layers.recurrent.GRULayer(network['input'],400, mask_input=network['mask'], gradient_steps=80, grad_clipping=100., only_return_final=True, backwards=True)

network['concat'] = ConcatLayer([network['gru-forward'],network['gru-backward']])
...
network['fc2'] = DenseLayer(network['concat'],5820,nonlinearity=None)
network['prob'] = NonlinearityLayer(network['fc2'],nonlinearity=softmax)

network_output = lasagne.layers.get_output(network['prob'])
  
hidden_output = lasagne.layers.get_output(network['concat'], deterministic=True)
